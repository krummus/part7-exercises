...
const AnecdoteList = ({ anecdotes }) => {
  ...
  if(id) {
    const paramAnecdote = anecdotes.find(n => n.id === Number(id))
    return(
      <div>
      <h2>{paramAnecdote.content}</h2>
      <p>has {paramAnecdote.votes} votes</p>
      <p>for more info see <a href={paramAnecdote.info}>{paramAnecdote.info}</a></p>
    </div>
    )
  }else{
    return (
